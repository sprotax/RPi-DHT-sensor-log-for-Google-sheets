...
2.)
Next we need to share the [Google spreadsheet](https://www.google.com/sheets/about/) with the bot. Look in your json file for a line labled `client_email:`, copy that email and share your Spreadsheet to that, remembering to give the bot edit permission.
<br>

3.) Before we get to far in we should wire up our sensor to our raspberry pi. The wiriing will differ slightly depending on if you have a built-in risitor or not. if you are not sure which type you have-look if its connected to a board, these ones also usally have a small led indicating power. If yours matchs either of the above descrition folllow the first section and if not, follow the seconed section.

* This section is for those who have a senosr with a built-in pullup resistor. These are very simple to connect, simply connect the pin labeled **VCC** to a 5 volt pinon the rpi, **GND** to a ground pin on the rpi, and lastly connect the **DATA** pin to any GPIO pin on the rpi.
* For those who don't have a built-in resistor with there sensor we will need to add one now. The resistor is required because the signal is a digital one and the rpi needs to be able to differentiate between the high and low signals. Connect the pins as follows, **VCC** to the 5 volt pin on the rpi, **GND** to any ground pin on the rpi. <br>
...
For those you want one long commmand instead, copy the below block.
```
...
<br>
In this folder your going to want to add your Google json authenication file. The simplist way to do that is to create a new file and then coping the infomation over. run `sudo nano gspread.json`, next open your Google json authenication file do a **ctrl+a** followed by **ctrl+c**. Then go back to your ssh application and right click in the window. this should copy and paste the infomation from your machine onto the raspberry pi.

5.) After that, we will configure the `config.json` file. Thihs will tell the scrip how you want it to run. <br>
...
* certification -- This is where we give the requird infomation to allow communication with the spreadsheet
    * json-authenentication -- This is the file location of the Google json authenication file, if you follewed the guide step by step it would be ./gspread.json
    * spreadsheet-name -- This is the title of the Google spreadsheet
* logger -- This all the infomation the script requires to run
    * DHT-type -- Is this a DHT11 or DHT22 senosr
    ...
    * interval -- Hoow long should it wait between running, measued in seconds.
...
7.) If you ever wish to stop the script either had a `#` infront of thhe line in crontab or add a file name **stop** in the Temp folder. to do this type `sudo nano stop` and **ctrl+s** followed by **ctrl+x**.
#### Example images
